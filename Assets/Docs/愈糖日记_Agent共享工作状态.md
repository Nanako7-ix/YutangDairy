...
本节为最新真实进度，优先级高于下方旧的“尚未完成”描述。

餐次与流程（已与用户确认）：

- Day 2 = 午餐；Day 3 = 早餐 + 运动（跑酷）；Day 4 = 晚餐。
- 已串联流程：Day2 采血四阶段 → `Day2_Stage4_SpaceMiniGame`（空格测血糖）→ `Day2_DietMatch_Lunch`（午餐）→ `Day3_DietMatch_Breakfast`（早餐）→ 后续 Day3 跑酷 / Day4 晚餐（待做）。

已完成（Unity 实测通过）：

- Day 2 采血流程拆为四个场景：`Day2_Stage1_PenAssembly` / `Day2_Stage2_InsertStrip` / `Day2_Stage3_Puncture` / `Day2_Stage4_SpaceMiniGame`。
- `Day2_Stage4` 为“按空格把指针保持在固定区间内累计 2 秒”的测血糖小游戏（横向进度条）。
- 饮食搭配小游戏（图片卡牌 UI，代码动态构建 uGUI，中文用系统字体）：
  - 通用脚本：`Assets/Scripts/Day2/Day2DietMatchController.cs`（名字是历史遗留，实为通用饮食搭配控制器；可配置标题/餐名/含糖量区间/卡牌/Sprite/下一场景）。
  - 玩法：选若干份食物卡（份数不限），总含糖量必须落在固定区间；满意度 = Σ 喜好（同一张卡重复份数喜好递减，默认 ×0.5）；满意度 × 系数 → 计入得分；达标额外给健康值奖励（可开关，默认 +5）。
...
说明：原 §6.2 / §6.3 / §6.4 中“选 3/4/5 张卡、类型平衡”等为旧设计，已被上述“含糖量区间 + 满意度 + 喜好递减、份数不限”的新设计取代。
...
4. Day 1 医院第三人称探索。
5. Day 2 血糖监测游戏化挑战和午餐饮食搭配。
6. Day 3 早餐饮食搭配和血糖平衡跑酷（运动）。
7. Day 4 晚餐饮食搭配，监测进阶，可选短版跑酷。
8. Day 5 四轨下落式音游。
9. Result 结算页。
10. 失败可重试，流程无阻断。
